...
def train(model, total_batch_size, queue, criterion, optimizer, device, train_begin, train_loader_count, print_batch=5, teacher_forcing_ratio=1):
    ...
    while True:
        ...
        # ★★★★★
        feats, scripts, feat_lengths, script_lengths = queue.get() # Mel로 바꾸면 여기서 Block!! 근데 Why??
        # ★★★★
...
def main():
    ...
    parser.add_argument('--hidden_size', type=int, default = 256, help='hidden size of model (default: 256)')  # 수정
    parser.add_argument('--encoder_layer_size', type=int, default = 5, help='number of layers of model (default: 5)')  # 수정
    parser.add_argument('--decoder_layer_size', type=int, default = 5, help='number of layers of model (default: 5)')  # 수정
    ...
    # 레이어 사이즈 차이를 저장하는 변수
    # EncoderRNN 생성자에 넘겨준다
    # 넘겨서 사이나는 만큼 없애줘야함
    if args.encoder_layer_size >= args.decoder_layer_size:
        diff_layer_size = args.encoder_layer_size - args.decoder_layer_size
    else:
        diff_layer_size = args.decoder_layer_size - args.encoder_layer_size

    enc = encoderRNN(feature_size, args.hidden_size,
                     input_dropout_p=args.dropout, dropout_p=args.dropout,
                     n_layers= args.encoder_layer_size,
                     bidirectional=args.bidirectional, rnn_cell='gru', variable_lengths=False, diff_layer_size = diff_layer_size)

    dec = decoderRNN(len(char2index), args.max_len, args.hidden_size * (2 if args.bidirectional else 1),
                     SOS_token, EOS_token,
                     n_layers=args.decoder_layer_size, rnn_cell='gru', bidirectional=args.bidirectional,
    ...
    # train_dataset_list 에 훈련데이터가 들어가 있다!!
    ...
    for epoch in range(begin_epoch, args.max_epochs):
        ...
        if epoch == 47:
            optimizer = optim.Adam(model.module.parameters(), lr = 0.00003 )

        if epoch == 70:
            optimizer = optim.Adam(model.module.parameters(), lr = 0.00001 )

        if epoch == 80:
            optimizer = optim.Adam(model.module.parameters(), lr = 0.000003)

        if epoch == 90:
            optimizer = optim.Adam(model.module.parameters(), lr = 0.000001)
