...
def train(model, total_batch_size, queue, criterion, optimizer, device, train_begin, train_loader_count, print_batch=5, teacher_forcing_ratio=1):
    ...
    #logger.info('16 구역 통과')

    model.train()

    #logger.info('17 구역 통과')

    #logger.info('train() start')

    begin = epoch_begin = time.time()

    #logger.info('18 구역 통과')
...
def bind_model(model, optimizer=None):
    ...
    def infer(wav_path):
        ...
        input = get_melspectrogram_feature(wav_path).unsqueeze(0)
        #input = get_melspectrogram_feature(wav_path).unsqueeze(0)
        logger.info('bind_model()의 infer() 진입')
...
def main():
    ...
    parser.add_argument('--hidden_size', type=int, default = 1024, help='hidden size of model (default: 256)')  #  수정
    parser.add_argument('--layer_size', type=int, default = 6, help='number of layers of model (default: 3)')   #  수정
    ...
    parser.add_argument('--max_epochs', type=int, default=10, help='number of max epochs in training (default: 10)') # 10 -> 1로 수정함!!!!!!
    ...
    #logger.info('11 구역 통과')

    logger.info('start')

    train_begin = time.time()

    #logger.info('12 구역 통과')

    for epoch in range(begin_epoch, args.max_epochs):

        #logger.info('13 구역 통과')
