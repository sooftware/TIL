 ...
 *  Title : 서버 채팅프로그램 
 *  Class : 서버 채팅 프로그램 서버 명령어 클래스 (UDP)
...
public class AdminCommand extends Thread implements ServerInterface{
	private SharedArea sArea;	// Thread 공유 메모리
	Thread sender, receiver;			// msg 전송 객체, 수신 객체
	...
	public int whatCommand(String command,Check ck){
		if(sArea.ck.checkServerExit(command)) return EXIT;			//	서버종료
		else if(sArea.ck.checkShow(command)) return SHOW;	    //	현재 유저 목록
		else if(sArea.ck.checkBanUser(command)) return BAN;		//	강퇴
		else if(sArea.ck.checkUserOut()) return USER_OUT; 		//	유저가 나갔는지
		else return ERROR;																		//	그 외
	}
